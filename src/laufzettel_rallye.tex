\documentclass[a4paper,10pt]{article}
\usepackage[utf8x]{inputenc}
\usepackage{geometry}
\usepackage{tabularx}
\usepackage{graphicx}

\usepackage{wallpaper}

\CenterWallPaper{0.8}{./inphima_final_grau}


\def\spiela{Info Fachschaft & Brücken bauen}
\def\spielb{5L & Twister}
\def\spielc{Draußen Mathe & Teilchenbeschleuniger}
\def\spield{25.22.00.72 & Inverses}
\def\spieldd{25.22.00.72 & Inverses}
\def\spiele{5K & Jeopardy}
\def\spielf{25.22.00.74 & Gefahrenstoffe}
\def\spielg{SP-Saal & Kleiderkette}
\def\spielh{5M & Memory}
\def\spielhh{Physik Fachschaft & Memory}
\def\spieli{Draußen Info & Extremsport}
\def\spielj{5J & Inphima-Duell}
%\def\pause{\multicolumn{2}{|c|}{\textbf{- PAUSE -}}}




\geometry{a4paper, top=15mm, left=15mm, right=15mm, bottom=15mm}
\renewcommand*\arraystretch{1.5}
%opening
\title{Laufzettel}
\author{In$\phi$Ma}
\date{06.10.2011}

\pagestyle{empty}



\newcommand{\oben}[1]{

\begin{tabularx}{\textwidth}{lc}
  \includegraphics[width=0.2\textwidth]{hhulogo_g}
& {\Huge \textbf{Laufzettel für Gruppe #1}}
\end{tabularx}
\large
\vspace{0.25cm} \\
\begin{tabularx}{\textwidth}{l}\textbf{Gruppenmitglieder:} \end{tabularx}\\ \phantom{blub}\\
\begin{tabularx}{\textwidth}{p{0.1\textwidth}p{0.4\textwidth}p{0.1\textwidth}p{0.4\textwidth}}
Name: & \underline{\hspace{6cm}} & Name: & \underline{\hspace{6cm}}\\
Name: & \underline{\hspace{6cm}} & Name: & \underline{\hspace{6cm}}\\
Name: & \underline{\hspace{6cm}} & Name: & \underline{\hspace{6cm}}\\
Name: & \underline{\hspace{6cm}} & Name: & \underline{\hspace{6cm}}\\
Name: & \underline{\hspace{6cm}} & Name: & \underline{\hspace{6cm}}\\
Name: & \underline{\hspace{6cm}} & Name: & \underline{\hspace{6cm}}\\
Name: & \underline{\hspace{6cm}} & Name: & \underline{\hspace{6cm}}\\
Name: & \underline{\hspace{6cm}} & Name: & \underline{\hspace{6cm}}\\
\end{tabularx}



}




\begin{document}

  \parindent 0pt

  \oben{1}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spiela			&		\\ \hline
    13:20 &\spielb			&		\\ \hline
    13:40 &\spielc			&		\\ \hline 
    14:00 &\spield		 	&		\\ \hline 
    14:20 &\spiele			&		\\ \hline
    14:40 &\spielf			&		\\ \hline
    15:00 &\spielg			&		\\ \hline
    15:20 &\spielh			&		\\ \hline
    15:40 &\spieli			&		\\ \hline
    16:00 &\spielj			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{2}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spielb			&		\\ \hline
    13:20 &\spielc			&		\\ \hline
    13:40 &\spield			&		\\ \hline 
    14:00 &\spiele		 	&		\\ \hline 
    14:20 &\spielf			&		\\ \hline
    14:40 &\spielg			&		\\ \hline
    15:00 &\spielh			&		\\ \hline
    15:20 &\spieli			&		\\ \hline
    15:40 &\spielj			&		\\ \hline
    16:00 &\spiela			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{3}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spielc			&		\\ \hline
    13:20 &\spield			&		\\ \hline
    13:40 &\spiele			&		\\ \hline 
    14:00 &\spielf		 	&		\\ \hline 
    14:20 &\spielg			&		\\ \hline
    14:40 &\spielh			&		\\ \hline
    15:00 &\spieli			&		\\ \hline
    15:20 &\spielj			&		\\ \hline
    15:40 &\spiela			&		\\ \hline
    16:00 &\spielb			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{4}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spield			&		\\ \hline
    13:20 &\spiele			&		\\ \hline
    13:40 &\spielf			&		\\ \hline 
    14:00 &\spielg		 	&		\\ \hline 
    14:20 &\spielh			&		\\ \hline
    14:40 &\spieli			&		\\ \hline
    15:00 &\spielj			&		\\ \hline
    15:20 &\spiela			&		\\ \hline
    15:40 &\spielb			&		\\ \hline
    16:00 &\spielc			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{5}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spiele			&		\\ \hline
    13:20 &\spielf			&		\\ \hline
    13:40 &\spielg			&		\\ \hline 
    14:00 &\spielh		 	&		\\ \hline 
    14:20 &\spieli			&		\\ \hline
    14:40 &\spielj			&		\\ \hline
    15:00 &\spiela			&		\\ \hline
    15:20 &\spielb			&		\\ \hline
    15:40 &\spielc			&		\\ \hline
    16:00 &\spield			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{6}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spielf			&		\\ \hline
    13:20 &\spielg			&		\\ \hline
    13:40 &\spielh			&		\\ \hline 
    14:00 &\spieli		 	&		\\ \hline 
    14:20 &\spielj			&		\\ \hline
    14:40 &\spiela			&		\\ \hline
    15:00 &\spielb			&		\\ \hline
    15:20 &\spielc			&		\\ \hline
    15:40 &\spield			&		\\ \hline
    16:00 &\spiele			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{7}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spielg			&		\\ \hline
    13:20 &\spielh			&		\\ \hline
    13:40 &\spieli			&		\\ \hline 
    14:00 &\spielj		 	&		\\ \hline 
    14:20 &\spiela			&		\\ \hline
    14:40 &\spielb			&		\\ \hline
    15:00 &\spielc			&		\\ \hline
    15:20 &\spield			&		\\ \hline
    15:40 &\spiele			&		\\ \hline
    16:00 &\spielf			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{8}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spielh			&		\\ \hline
    13:20 &\spieli			&		\\ \hline
    13:40 &\spielj			&		\\ \hline 
    14:00 &\spiela		 	&		\\ \hline 
    14:20 &\spielb			&		\\ \hline
    14:40 &\spielc			&		\\ \hline
    15:00 &\spield			&		\\ \hline
    15:20 &\spiele			&		\\ \hline
    15:40 &\spielf			&		\\ \hline
    16:00 &\spielg			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{9}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spieli			&		\\ \hline
    13:20 &\spielj			&		\\ \hline
    13:40 &\spiela			&		\\ \hline 
    14:00 &\spielb		 	&		\\ \hline 
    14:20 &\spielc			&		\\ \hline
    14:40 &\spield			&		\\ \hline
    15:00 &\spiele			&		\\ \hline
    15:20 &\spielf			&		\\ \hline
    15:40 &\spielg			&		\\ \hline
    16:00 &\spielh			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{10}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spielj			&		\\ \hline
    13:20 &\spiela			&		\\ \hline
    13:40 &\spielb			&		\\ \hline 
    14:00 &\spielc		 	&		\\ \hline 
    14:20 &\spield			&		\\ \hline
    14:40 &\spiele			&		\\ \hline
    15:00 &\spielf			&		\\ \hline
    15:20 &\spielg			&		\\ \hline
    15:40 &\spielh			&		\\ \hline
    16:00 &\spieli			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{11}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spiela			&		\\ \hline
    13:20 &\spielb			&		\\ \hline
    13:40 &\spielc			&		\\ \hline 
    14:00 &\spieldd		 	&		\\ \hline 
    14:20 &\spiele			&		\\ \hline
    14:40 &\spielf			&		\\ \hline
    15:00 &\spielg			&		\\ \hline
    15:20 &\spielhh			&		\\ \hline
    15:40 &\spieli			&		\\ \hline
    16:00 &\spielj			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{12}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spielb			&		\\ \hline
    13:20 &\spielc			&		\\ \hline
    13:40 &\spieldd			&		\\ \hline 
    14:00 &\spiele		 	&		\\ \hline 
    14:20 &\spielf			&		\\ \hline
    14:40 &\spielg			&		\\ \hline
    15:00 &\spielhh			&		\\ \hline
    15:20 &\spieli			&		\\ \hline
    15:40 &\spielj			&		\\ \hline
    16:00 &\spiela			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{13}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spielc			&		\\ \hline
    13:20 &\spieldd			&		\\ \hline
    13:40 &\spiele			&		\\ \hline 
    14:00 &\spielf		 	&		\\ \hline 
    14:20 &\spielg			&		\\ \hline
    14:40 &\spielhh			&		\\ \hline
    15:00 &\spieli			&		\\ \hline
    15:20 &\spielj			&		\\ \hline
    15:40 &\spiela			&		\\ \hline
    16:00 &\spielb			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{14}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spieldd			&		\\ \hline
    13:20 &\spiele			&		\\ \hline
    13:40 &\spielf			&		\\ \hline 
    14:00 &\spielg		 	&		\\ \hline 
    14:20 &\spielhh			&		\\ \hline
    14:40 &\spieli			&		\\ \hline
    15:00 &\spielj			&		\\ \hline
    15:20 &\spiela			&		\\ \hline
    15:40 &\spielb			&		\\ \hline
    16:00 &\spielc			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{15}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spiele			&		\\ \hline
    13:20 &\spielf			&		\\ \hline
    13:40 &\spielg			&		\\ \hline 
    14:00 &\spielhh		 	&		\\ \hline 
    14:20 &\spieli			&		\\ \hline
    14:40 &\spielj			&		\\ \hline
    15:00 &\spiela			&		\\ \hline
    15:20 &\spielb			&		\\ \hline
    15:40 &\spielc			&		\\ \hline
    16:00 &\spieldd			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{16}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spielf			&		\\ \hline
    13:20 &\spielg			&		\\ \hline
    13:40 &\spielhh			&		\\ \hline 
    14:00 &\spieli		 	&		\\ \hline 
    14:20 &\spielj			&		\\ \hline
    14:40 &\spiela			&		\\ \hline
    15:00 &\spielb			&		\\ \hline
    15:20 &\spielc			&		\\ \hline
    15:40 &\spieldd			&		\\ \hline
    16:00 &\spiele			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{17}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spielg			&		\\ \hline
    13:20 &\spielhh			&		\\ \hline
    13:40 &\spieli			&		\\ \hline 
    14:00 &\spielj		 	&		\\ \hline 
    14:20 &\spiela			&		\\ \hline
    14:40 &\spielb			&		\\ \hline
    15:00 &\spielc			&		\\ \hline
    15:20 &\spieldd			&		\\ \hline
    15:40 &\spiele			&		\\ \hline
    16:00 &\spielf			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{18}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spielhh			&		\\ \hline
    13:20 &\spieli			&		\\ \hline
    13:40 &\spielj			&		\\ \hline 
    14:00 &\spiela		 	&		\\ \hline 
    14:20 &\spielb			&		\\ \hline
    14:40 &\spielc			&		\\ \hline
    15:00 &\spieldd			&		\\ \hline
    15:20 &\spiele			&		\\ \hline
    15:40 &\spielf			&		\\ \hline
    16:00 &\spielg			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{19}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spieli			&		\\ \hline
    13:20 &\spielj			&		\\ \hline
    13:40 &\spiela			&		\\ \hline 
    14:00 &\spielb		 	&		\\ \hline 
    14:20 &\spielc			&		\\ \hline
    14:40 &\spieldd			&		\\ \hline
    15:00 &\spiele			&		\\ \hline
    15:20 &\spielf			&		\\ \hline
    15:40 &\spielg			&		\\ \hline
    16:00 &\spielhh			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  \oben{20}
  \LARGE
  \vspace{1cm}
  \begin{tabularx}{\textwidth}{p{2.5cm}||p{4.5cm}|p{6.5cm}|l}
    \textbf{Zeit}  & \textbf{Ort} 	&\textbf{Titel} & \textbf{Ergebnis} 	\\ \hline \hline
    13:00 &\spielj			&		\\ \hline
    13:20 &\spiela			&		\\ \hline
    13:40 &\spielb			&		\\ \hline 
    14:00 &\spielc		 	&		\\ \hline 
    14:20 &\spieldd			&		\\ \hline
    14:40 &\spiele			&		\\ \hline
    15:00 &\spielf			&		\\ \hline
    15:20 &\spielg			&		\\ \hline
    15:40 &\spielhh			&		\\ \hline
    16:00 &\spieli			&		\\ \hline 
    
  \end{tabularx}


  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


  \newpage

  
\end{document}
